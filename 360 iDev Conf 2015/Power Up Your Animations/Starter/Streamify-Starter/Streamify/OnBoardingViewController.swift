    ...
    func adjustHeights(viewToSelect : UIView) {
        for constraint in viewToSelect.superview!.constraints() as! [NSLayoutConstraint]{
            if constraint.identifier == "StripHeight" {
                ...
                let con = NSLayoutConstraint(item: constraint.firstItem, attribute: .Height, relatedBy: .Equal, toItem: constraint.secondItem, attribute: .Height, multiplier: multiplier, constant: 0)
